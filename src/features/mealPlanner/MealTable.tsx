...
const MealTable = ({targetMealPlan, handleAddClick, handleRecipeSelect}: Props) => {
	...
	const renderRecipes = (recipes : RecipeDoc[]): React.ReactNode => {
		return (
			<div className="row-recipe-wrapper">
				{recipes.map((recipe: RecipeDoc) => <Link key={`recipe-${recipe._id}`} linkText={recipe.name} handleClick={() => handleRecipeSelect(recipe._id)} />)}
			</div>
		)
	}
